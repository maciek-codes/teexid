...
import { Box, Button, Grid, GridItem, HStack, Input, Stack, Text } from "@chakra-ui/react";
...
const StoryPromptInput: React.FC<StoryPromptInputProps> = ({selectedCard}) => {
  const ws = useSocket();
  ...
  return (
    <Stack>
      <HStack>
        ...
        <Input type="text" placeholder="Be creative!" onChange={(e) => setStoryText(e.currentTarget.value)} />
      </HStack>
      <Button isActive={storyText.trim() !== "" && selectedCard !== null} 
        onClick={() => submitStory()}>Submit story</Button>
    </Stack>
  )
}
...
export const Game: React.FC = () => {
  ...
  const onMessage = useCallback((evt: MessageEvent<string>) => {
  ...
  return (
    <Box>
      <Grid templateColumns="repeat(5, 1fr)" 
        ...
        <GridItem colStart={0} rowStart={0} colSpan={5} rowSpan={1} alignContent='start' justifyContent="start">
        ...
        <GridItem colStart={0} colSpan={3} rowStart={1} rowSpan={4}>
          ...
          <CardSelector cards={cards} onSelected={(selectedCard ) => {
